...
import { AArrowDown, Github, Mail } from "lucide-react";
  ...
  return (
    <section className="relative h-screen flex items-center justify-center overflow-hidden bg-linear-to-br from-white via-gray-50 to-gray-100 pt-20">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 text-center">
        <div className={`translation-all duration-1000 ${hasAnimated.hero ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`} id="hero">
          <div className="mb-4">
            <span className="inline-block px-4 py-2 bg-black text-white rounded-full text-sm font-medium animate-fade-in">
              {heroData.greeting}
            </span>
          </div>
          <h1 className="text-5xl md:text-7xl leading-tight font-bold mb-4 bg-linear-to-r from-black via-gray-800 to-gray-600 bg-clip-text text-transparent animate-fade-in-up">{heroData.name}</h1>
          <div className="h-12 md:h-16 mb-6 flex items-center justify-center">
            <h2 className="text-2xl md:text-4xl font-semibold text-gray-700"> I'm a
              <span className="relative">
                <span className="text-blue-600 font-bold">
                  {currentText}
                  <span className="animate-pulse"></span>
                </span>
                <span className="absolute -bottom-1 left-1 w-full h-0.5 bg-linear-to-r from-blue-500 to-purple-600 "></span>
              </span>
            </h2>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8 animate-fade-in-up delay-300">
            {heroData.ctaButton.map((button, index) => (
          ...
          <div className="flex justify-center space-x-6 mb-8 animate-fade-in-up delay-400">
            {heroData.socialLinks.map((social, index) => {
              const IconComponent = social.icon === "GitHub" ? Github : social.icon === "Telegram" ? TelegramIcon : Mail;
              return (
                <a key={index} href={social.url} target="_blank" rel="noopener noreferrer" className="group p-3 bg-white rounded-full shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-110 hover:-translate-y-1">
                  <IconComponent className="w-6 h-6 text-gray-700 group-hover:text-black transition-colors " />
                </a>
              )
            })}
          </div>
          <div className="grid grid-cols-2 gap-4 max-w-2xl mx-auto mb-8 animate-fade-in-up delay-500">
